...
// encryptVer = "9.1"
function getckey(platform, appVer, vid, empty_str="", guid, tm, _path){
    var _args = [platform, appVer, vid, empty_str="", guid, tm];
    var c = ['number', 'string', 'string', 'string', 'string', 'number'];
    wasm_data = fs.readFileSync(_path+'/js/tx-ckey.wasm')

    buffer = new Uint8Array(wasm_data);

    wasmobject = new WebAssembly.Instance(new WebAssembly.Module(buffer), importObject);
    return i('getckey', 'string', c, _args, undefined)

}
var platform = process.argv[2];
var appVer = process.argv[3];
var vid = process.argv[4];
//var empty_str = process.argv[5];
var guid = process.argv[5];
var tm = process.argv[6];
var _path = process.argv[7];
...
//console.log(tm)
//console.log(_path)
console.log(getckey(platform, appVer, vid, '', guid, tm, _path))
